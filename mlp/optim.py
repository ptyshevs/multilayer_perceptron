...
class GradientDescent(Optimizer):
    def __init__(self, lr=.05):
        self.learning_rate = learning_rate_mapper(lr)
...
class MomentumGradientDescent(Optimizer):
    def __init__(self, lr=.05, beta=.9, bias_correction=True):
        self.learning_rate = learning_rate_mapper(lr)
        self.beta = beta
...
class RMSProp(Optimizer):
    def __init__(self, lr=.05, beta=.9, eps=1e-5):
    ...
    def __call__(self, layer, params_grad, **kwargs):
        ...
        for i, p in enumerate(layer.params):
            ...
            self.moments[layer][i] += (1. - self.beta) * (params_grad[i] / batch_size) ** 2
            
            grad_scale = lr / np.sqrt(self.moments[layer][i] + self.eps)
            
            layer.params[i] -= grad_scale * params_grad[i] / batch_size


class Adam(Optimizer):
    def __init__(self, lr=.01, beta1=.9, beta2=.999, eps=1e-4, clipvalue=None):
        self.lr = learning_rate_mapper(lr)
        self.beta1 = beta1
        self.beta2 = beta2
        self.eps = eps
    ...
    def __call__(self, layer, params_grad, **kwargs):
        ...
        for i, p in enumerate(layer.params):
            ...
            self.moments[layer][i] += (1 - self.beta1) * params_grad[i] / batch_size
            
            self.norms[layer][i] *= self.beta2
            self.norms[layer][i] += (1 - self.beta2) * (params_grad[i] / batch_size) ** 2
            
            # Bias correction on both
            moment_corrected = self.moments[layer][i]
            if self.beta1 < 1:
